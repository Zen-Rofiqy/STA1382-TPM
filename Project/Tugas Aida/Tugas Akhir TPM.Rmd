...
set.seed(1401211016)
library(rsample) 
split <- initial_split(data.a, prop = 0.8)
data_train <- training(split)
data_test <- testing(split)

df_split <- rbind(
data.frame(table(data_train$Y),"type"="train"),
data.frame(table(data_test$Y),"type"="test")) %>% 
  mutate(
    Var1 = as.factor(Var1),
    type = as.factor(type)
  ) 
...
ggplot(df_split, aes(x=Var1,y=Freq, fill=type))+
  ...
  labs(x = "CO2 Emissions", 
       y = "Frequency", 
       fill = "Data", 
       title = "CO2 Emissions: Proportion Train vs Test")+
  theme_minimal()+
  theme(
...
rf_model <- randomForest(Y ~ . ,
                         data = TRAIN,
                         ntree= 550, # Banyaknya pohon
                         mtry=3, # Jumlah variable yang dipertimbangkan perpohon
                         keep.forest=TRUE, # simpan pohon dlm hutan
                         importance=TRUE) # hitung & simpan rank importance
