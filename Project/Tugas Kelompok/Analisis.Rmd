...
data_kmeans <- kmeans(x = dt, centers = 2)
fviz_cluster(object = data_kmeans, data=dt)
```

K-means berhasil membuat cluster berdasarkan kedekatan jaraknya.
...
Proporsi jumlah cluster 1 dan 2 adalah 7 : 27.
...
Berlainan dengan hasil dari GMM, cluster 1 merupakan cluster dengan aspek kesejahteraan yang paling tinggi dibandingkan dengan cluster 2. Benar-benar unggul semua. Mari kita lihat lebih jelas dengan radar chart dibawah.
...
Sejalan dengan yang saya bilang diatas, bahwa cluster 1 benar-benar mengungguli cluster 2.
...
ggplot() +  
  geom_sf(data=data.indo, aes(fill=factor(`Cluster`))) +
  scale_fill_manual(values=c("1" = "lightgreen", "2" = "indianred"), 
                    name = "Keterangan") +
...
Terlihat bahwa 7 provinsi tersebut merupakan provinsi yang mungkin blm dapat dikelompokkan secara benar oleh FCM.

Walau demikian, FCM tidak mampu untuk membuat cluster lebih dari 2 pada kasus ini, tadi saya sudah mencobanya. Sehingga ini adalah hasil terbaik dari FCM
