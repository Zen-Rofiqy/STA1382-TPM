...
```{r echo=FALSE}
rf <- readRDS("rf1.rds"); rf
```

Hasil validasi silang ditampilkan pada plot berikut:

```{r fig.height=15, fig.width=20, message=FALSE, warning=FALSE, dpi=300}
...
rf_tg <- readRDS("rf_tg1.rds"); rf_tg
```
...
```{r echo=FALSE}
rf_tg <- readRDS("rf_tg2.rds"); rf_tg
```
